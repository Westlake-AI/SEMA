...
class BaseSwitchEMAHook(Hook):
    """Switch Exponential Moving Average Hook.
    ...
    def after_train_iter(self, runner):
        ...
        if self.switch_params and self.switch_by_iter:
            if runner.iter > self.switch_start:
                if (runner.iter + 1) % self.switch_interval != 0:
                    return
    ...
    def before_train_epoch(self, runner):
        ...
        if self.switch_params and not self.switch_by_iter:  # copy EMA to the model
            if runner.epoch > self.switch_start:
                if (runner.epoch + 1) % self.switch_interval != 0:
                    return
                self._switch_ema_parameters()
...
class ExpMomentumSwitchEMAHook(BaseSwitchEMAHook):
    ...
    def __init__(self, total_iter=2000, **kwargs):
        super(ExpMomentumSwitchEMAHook, self).__init__(**kwargs)
        self.momentum_fun = lambda x: (1 - self.momentum) * math.exp(-(
            1 + x) / total_iter) + self.momentum


@HOOKS.register_module()
class LinearMomentumSwitchEMAHook(BaseSwitchEMAHook):
    ...
    def __init__(self, warm_up=100, warmup_ratio=0.9, **kwargs):
        super(LinearMomentumSwitchEMAHook, self).__init__(**kwargs)
        assert 0 < self.momentum < 0.5
        self.momentum_fun = lambda x: max(self.momentum,
                            (1-self.momentum) * (1 - x / warm_up) * (1 - warmup_ratio))
